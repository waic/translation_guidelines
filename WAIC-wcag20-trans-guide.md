...
### 常用漢字表の本表の「音訓」に示されない表記が適用される可能性がある語は、かなとします。
...
1. 原則として、半角英数字と全角文字との間は半角スペースを入れる。ただし、次のいずれかに該当する場合を除く (すなわち、半角スペースを入れない):
    1. 文字+半角コロンの組み合わせの場合
    1. 半角英数字と句読点が連続する場合
    1. 対になる半角記号の内側の場合
2. 全角の記号の前後は半角スペースを入れない
...
1.iii.の例：`境界内 (ビューポート) に収まる`、`例えば "このような" 表現`

2.の例：`オプションは「ハンバーガー」メニューボタン`

## キーボードのキー表記について
原則として、106/109キーボードの刻印に従って表記します。ただし、スペースキーは無刻印なので、カナで表記します。[Template:106キーボードの配列 - Wikipedia](https://ja.wikipedia.org/wiki/Template:106%E3%82%AD%E3%83%BC%E3%83%9C%E3%83%BC%E3%83%89%E3%81%AE%E9%85%8D%E5%88%97)も参照。
